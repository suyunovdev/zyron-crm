...
// ── Lidlar tahlili (marketing: qayerdan kelgan) ──
interface LeadStats {
...
export function LeadStatsTab() {
  ...
  useEffect(() => { fetch('/api/superadmin/lead-stats').then(r => r.json()).then(setD); }, []);
  if (!d) return <Loading />;
  ...
  const statusBars = ['new', 'contacted', 'trial', 'enrolled', 'rejected'].map(k => ({ label: LEAD_STATUS_LABELS[k], value: d.byStatus[k] || 0 }));
  const kpis = [
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={card}>
        <p className="text-sm font-semibold text-slate-700 mb-3">Manba bo&apos;yicha — qayerdan bildi</p>
        <Donut data={donut} />
      </div>
      <div className="grid md:grid-cols-2 gap-4">
        <div className={card}>
          <p className="text-sm font-semibold text-slate-700 mb-3">Manba — soni</p>
          <Bars data={sourceBars} />
        </div>
        <div className={card}>
          <p className="text-sm font-semibold text-slate-700 mb-3">Filial bo&apos;yicha</p>
          <Bars data={d.byBranch.map(b => ({ label: b.label, value: b.count }))} />
        </div>
      </div>
      <div className={card}>
        <p className="text-sm font-semibold text-slate-700 mb-3">Oylik trend (6 oy)</p>
        <LineArea data={trend} />
      </div>
      <div className={card}>
        <p className="text-sm font-semibold text-slate-700 mb-3">Holat (funnel)</p>
